...
var _zubat = require("zubat");

var _zubat2 = _interopRequireDefault(_zubat);
...
var compile = function compile(task, opts) {
  ...
  var job = (0, _zubat2.default)(themeDir, opts, function (err) {
    if (err) {
      task.fail(err);
    } else {
    ...
    switch (sev) {
      case "error":
        job.cleanup(function () {
          return task.fail("Zubat fainted. " + str);
        });
        break;
      case "warning":
        job.cleanup(function () {
          return task.fail("zubat: " + str);
        });
        break;
      default:
        task.info("zubat: " + str);
