...
var check = function check(task, _ref) {
  ...
  (0, _git2.default)('remote show basetheme', 'Checking if base theme remote exists...', gopts).catch(function () {
    ...
    if (!fallbackCommit) {
      throw new Error('Could not find a merge base with the base theme ' + 'repository, or a baseThemeVersion specified in ' + 'theme.json. Please check your `basetheme` remote.');
    }
